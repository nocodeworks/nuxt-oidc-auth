...
function callbackEventHandler({ onSuccess }: OAuthConfig<UserSession>) {
  ...
  return eventHandler(async (event: H3Event) => {
    ...
    // DPoP (RFC 9449, subissue 013): when enabled, generate a session-scoped ES256
    // keypair on the Nitro server and attach an initial DPoP proof header to the
    // token request. The AS validates the proof, computes the RFC 7638 thumbprint
    // of the proof's `jwk`, and embeds it into the access token as `cnf.jkt`.
    // Resource servers later match this `jkt` against fresh proofs generated by
    // the BFF proxy. Browsers never see the keypair — the private half is AES-GCM
    // encrypted with NUXT_OIDC_TOKEN_KEY and persisted only in Nitro storage.
    ...
    if (config.dpopEnabled) {
      ...
      ;(headers as Record<string, string>).DPoP = dpopProof
    }
    ...
    if (
      ...
      || config.exposeAccessToken
      || config.exposeIdToken
      || dpopKeypair
    ) {
